"""
这个脚本用于在火星臭氧预测任务上比较两组 iTransformer 风格实验：
1. iTransformer_Raw
   不使用相位扭曲模块，直接把历史原始变量送入 iTransformer 主干。
2. iTransformer_PhaseWarp
   先经过 PhaseWarpFrontEnd，再送入完全相同的 iTransformer 主干。

设计目标：
- 尽量保持与 dlinear_phasewarp_compare.py 相同的数据切分、标准化和评价口径；
- 采用“格点共享的点序列建模”方式，让 iTransformer 更符合它原本的多变量时序设定；
- 保留 iTransformer 的核心思想：把“变量”而不是“时间步”当作 token，
  通过变量维自注意力学习多变量相关性；
- 只改变是否插入 Phase Warp 前端，使性能差异更容易归因到你的创新模块。

说明：
- 这里实现的是适合当前项目的轻量 iTransformer 风格版本；
- 核心结构参考了 THUML 官方 iTransformer 的 encoder-only 思路：
  “B L N -> B N E -> Encoder -> Linear -> B pred_len N”。
"""
...
class InvertedDataEmbedding(nn.Module):
    ...
    这里的做法是：
    1. 先把每个变量自己的整段历史 [seq_len] 看作一个向量；
    2. 用线性层把它映射到 d_model；
    3. 再加上变量 token 的可学习嵌入。
    """

    def __init__(self, seq_len, n_vars, d_model, dropout):
        super().__init__()
        self.value_embedding = nn.Linear(seq_len, d_model, bias=False)
        self.token_embedding = nn.Parameter(torch.randn(1, n_vars, d_model) * 0.02)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        # x: [B_like, seq_len, n_vars]
        x = x.permute(0, 2, 1)
        x = self.value_embedding(x) + self.token_embedding
        return self.dropout(x)


class ITransformerBackbone(nn.Module):
    """
    一个适合当前项目的轻量 iTransformer 主干。

    结构思路与官方版本一致：
    1. 可选做逐样本归一化；
    2. 把多变量序列从 [B, L, N] 变成 [B, N, E]；
    3. 在变量维 token 上做 TransformerEncoder；
    4. 用线性层把每个变量 token 投影成未来 pred_len 步；
    5. 输出 [B, pred_len, N]。
    """

    def __init__(self, seq_len, pred_len, n_vars, d_model, n_heads, e_layers, d_ff, dropout, use_norm=True):
        super().__init__()
        ...
        self.embedding = InvertedDataEmbedding(seq_len=seq_len, n_vars=n_vars, d_model=d_model, dropout=dropout)
    ...
    def forward(self, x):
        ...
        # [B_like, seq_len, n_vars] -> [B_like, n_vars, d_model]
        x = self.embedding(x)
        x = self.encoder(x)
        x = self.encoder_norm(x)

        # [B_like, n_vars, d_model] -> [B_like, pred_len, n_vars]
...
class GridPointITransformerO3(nn.Module):
    """
    把每个经纬度格点看成一条多变量时间序列，并共享同一个 iTransformer 模型。
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, d_model, n_heads, e_layers, d_ff, dropout):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
...
def train_and_evaluate(
    ...
    """
    训练一组 iTransformer 实验，并返回评估指标。
    受控变量只有一个：是否启用 phase-warp 前端。
    """

    print(f"\n[Experiment] {label}")
    model = GridPointITransformerO3(
        seq_len=window,
        pred_len=horizon,
        lat_size=lat_size,
        lon_size=lon_size,
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, "models", "训练过程", "iTransformer_PhaseWarp_Compare.txt"))
