"""
这个脚本用于在火星臭氧预测任务上比较两组 Autoformer 风格实验：
1. Autoformer_Raw
   不使用相位扭曲模块，直接把历史原始变量送入 Autoformer 主干。
2. Autoformer_PhaseWarp
   先经过 PhaseWarpFrontEnd，再送入完全相同的 Autoformer 主干。

设计目标：
- 尽量保持与前面其他 compare 脚本一致的数据切分、标准化和评估口径；
- 采用“格点共享的点序列建模”方式，让 Autoformer 更符合它的多变量时序建模设定；
- 保留 Autoformer 最核心的思想：序列分解 + 分解后建模；
- 只改变“是否插入 Phase Warp 前端”，让性能差异更容易归因到你的相位扭曲创新。

说明：
- 这里实现的是适合当前项目的轻量 Autoformer 风格版本，不是官方仓库的逐行复刻；
- 但 Autoformer 最关键的“moving-average decomposition + trend/seasonal forecasting”
  逻辑被保留了下来，可以作为论文里的有效对比基线。
"""
...
class EarlyStopping:
    """
    基于验证集损失的简单早停器。
    这个 compare 脚本的目标是做受控实验，因此早停逻辑保持简单透明即可。
    """
...
def clean_invalid(x, name):
    """把 NaN / inf / 异常值替换成 0，避免训练阶段数值不稳定。"""
...
class MovingAvg(nn.Module):
    """
    Autoformer 里的基础模块之一：移动平均。
    它用于把时间序列的低频趋势成分平滑提取出来。
    """
...
class SeriesDecomp(nn.Module):
    """
    序列分解模块：
    - trend: 通过 moving average 得到的平滑趋势项
    - seasonal: 原序列减去趋势项后得到的波动项
    """
...
class AutoformerBackbone(nn.Module):
    """
    适合当前项目的轻量 Autoformer 风格主干。

    这里保留了 Autoformer 的核心思想：
    - 先把输入序列分解为 seasonal 和 trend；
    - seasonal 走编码层做深度时序建模；
    - trend 走更直接的线性外推；
    - 最后两部分相加形成未来预测。
    """

    def __init__(self, seq_len, pred_len, n_vars, d_model, n_heads, e_layers, d_ff, moving_avg, dropout):
        super().__init__()
        ...
        # 官方 Autoformer 的一个标志性设计是不强调传统的位置编码，
        # 这里也采用更简洁的 value embedding。
    ...
    def forward(self, x):
        ...
        # seasonal_hidden: [B_like, seq_len, d_model]
        ...
        # trend_init: [B_like, seq_len, n_vars] -> [B_like, pred_len, n_vars]
...
class GridPointAutoformerO3(nn.Module):
    """
    把每个经纬格点看成一条多变量时间序列，并共享同一个 Autoformer 模型。
...
def train_and_evaluate(
    ...
    """
    训练一组 Autoformer 实验，并返回评估指标。
    受控变量只有一个：是否启用 phase-warp 前端。
    """

    print(f"\n[Experiment] {label}")
    model = GridPointAutoformerO3(
        seq_len=window,
        pred_len=horizon,
        lat_size=lat_size,
        lon_size=lon_size,
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, "models", "训练过程", "Autoformer_PhaseWarp_Compare.txt"))
