"""
这个脚本用于在火星臭氧预测任务上比较两组 N-BEATS 风格实验：
1. NBEATS_Raw
   不使用相位扭曲模块，直接把历史原始变量送入 N-BEATS 主干。
2. NBEATS_PhaseWarp
   先经过 PhaseWarpFrontEnd，再送入完全相同的 N-BEATS 主干。

设计目标：
- 尽量保持与前面其他 compare 脚本一致的数据切分、标准化和评估口径；
- 采用“格点共享的点序列建模”方式，让 N-BEATS 可以直接作为多变量时间序列强基线；
- 保留 N-BEATS 的核心思想：残差回看 backcast + 逐块累加 forecast；
- 只改变“是否插入 Phase Warp 前端”，让性能差异更容易归因到你的相位扭曲创新。

说明：
- 这里实现的是适合当前项目的轻量 N-BEATS 风格版本，不是官方仓库的逐行复刻；
- 但 N-BEATS 最有代表性的 generic stack 逻辑被保留了下来：
  每个 block 负责解释一部分历史残差，并贡献一部分未来预测。
"""
...
class NBeatsBlock(nn.Module):
    ...
    每个 block 接收当前还未解释完的历史输入 residual，
    然后输出两部分：
    - backcast：解释掉的一部分历史
    - forecast：对未来的一部分贡献

    这正是 N-BEATS 最核心的“逐块解释历史、逐块累加未来”的思想。
    """

    def __init__(self, input_size, forecast_size, hidden_dim, n_layers, dropout):
        super().__init__()
        layers = []
        in_dim = input_size
...
class NBeatsBackbone(nn.Module):
    """
    适合当前项目的轻量 N-BEATS 风格主干。

    流程：
    1. 把多变量历史窗口展平成一个长向量；
    2. 多个 block 依次处理 residual；
    3. 每个 block 解释一部分历史 backcast，并叠加一部分未来 forecast；
    4. 最终把 forecast reshape 成 [pred_len, n_vars]。
    """

    def __init__(self, seq_len, pred_len, n_vars, stack_count, blocks_per_stack, hidden_dim, n_layers, dropout):
        super().__init__()
        ...
        total_blocks = stack_count * blocks_per_stack
...
class GridPointNBeatsO3(nn.Module):
    """
    把每个经纬格点看成一条多变量时间序列，并共享同一个 N-BEATS 模型。
...
def train_and_evaluate(
    ...
    """
    训练一组 N-BEATS 实验，并返回评估指标。
    受控变量只有一个：是否启用 phase-warp 前端。
    """

    print(f"\n[Experiment] {label}")
    model = GridPointNBeatsO3(
        seq_len=window,
        pred_len=horizon,
        lat_size=lat_size,
        lon_size=lon_size,
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, "models", "训练过程", "NBEATS_PhaseWarp_Compare.txt"))
