"""
这个脚本用于在火星臭氧预测任务上比较两组 Informer 风格实验：
1. Informer_Raw
   不使用相位扭曲模块，直接把历史原始变量送入 Informer 主干。
2. Informer_PhaseWarp
   先经过 PhaseWarpFrontEnd，再送入完全相同的 Informer 主干。

设计目标：
- 尽量保持与前面其他 compare 脚本一致的数据切分、标准化和评估口径；
- 采用“格点共享的点序列建模”方式，让 Informer 更符合它的多变量时序预测设定；
- 保留 Informer 最有代表性的两个思想：
  1. attention distilling：逐层压缩时间维表示；
  2. generative-style one-shot forecasting：一次前向直接输出未来序列；
- 只改变“是否插入 Phase Warp 前端”，让性能差异更容易归因到你的相位扭曲创新。

说明：
- 这里实现的是适合当前项目的轻量 Informer 风格版本，不是官方仓库逐行复刻；
- 但它保留了 Informer 在长序列预测里最核心的编码思路，可以作为论文中的有效对比基线。
"""
...
class EarlyStopping:
    """
    基于验证集损失的简单早停器。
    这类 compare 脚本主要目标是做受控实验，因此这里保持简单透明。
    """
...
class DistillConvLayer(nn.Module):
    """
    Informer 的 attention distilling 模块。

    作用：
    - 在一层注意力建模后，用卷积和池化压缩时间维；
    - 保留主导时序模式，同时减少后续层的计算和冗余。
    """

    def __init__(self, d_model):
        super().__init__()
        self.down_conv = nn.Conv1d(d_model, d_model, kernel_size=3, padding=1)
...
class InformerEncoderLayer(nn.Module):
    ...
    这里保留标准 Transformer 编码层骨架：
    - self-attention
    - residual + norm
    - FFN
    - residual + norm

    真正把它和普通 Transformer 拉开的，是层间额外接的 distilling 模块。
    """
...
class InformerBackbone(nn.Module):
    """
    适合当前项目的轻量 Informer 风格主干。

    保留了 Informer 的两点核心直觉：
    1. 编码层之间用 distilling 压缩时间长度；
    2. 最终不做逐步自回归，而是一次前向直接生成未来 pred_len 步。
    """

    def __init__(self, seq_len, pred_len, n_vars, d_model, n_heads, e_layers, d_ff, dropout):
        super().__init__()
        ...
        self.encoder_layers = nn.ModuleList(
            [InformerEncoderLayer(d_model=d_model, n_heads=n_heads, d_ff=d_ff, dropout=dropout) for _ in range(e_layers)]
        )
        self.distill_layers = nn.ModuleList([DistillConvLayer(d_model=d_model) for _ in range(max(0, e_layers - 1))])
        self.norm = nn.LayerNorm(d_model)

        # 根据 distilling 次数推断最终序列长度，方便后面展平做 one-shot forecasting。
        final_len = seq_len
        for _ in range(max(0, e_layers - 1)):
            final_len = max(1, (final_len + 1) // 2)
        self.final_len = final_len

        self.head = nn.Sequential(
            nn.Flatten(start_dim=1),
            nn.Linear(self.final_len * d_model, pred_len * n_vars),
        )
    ...
    def forward(self, x):
        ...
        x = self.value_embedding(x) + self.position_embedding[:, : self.seq_len, :]
        x = self.dropout(x)

        for layer_idx, layer in enumerate(self.encoder_layers):
            x = layer(x)
            if layer_idx < len(self.distill_layers):
                x = self.distill_layers[layer_idx](x)

        x = self.norm(x)
        x = self.head(x).view(-1, self.pred_len, self.n_vars)

        # one-shot 输出后，再把内部归一化还原回原量纲。
        x = x * stdev[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1)
        x = x + means[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1)
        return x


class GridPointInformerO3(nn.Module):
    """
    把每个经纬格点看成一条多变量时间序列，并共享同一个 Informer 模型。
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, d_model, n_heads, e_layers, d_ff, dropout):
        ...
        self.backbone = InformerBackbone(
            seq_len=seq_len,
            pred_len=pred_len,
            n_vars=n_vars,
            d_model=d_model,
...
def train_and_evaluate(
    ...
    """
    训练一组 Informer 实验，并返回评估指标。
    受控变量只有一个：是否启用 phase-warp 前端。
    """

    print(f"\n[Experiment] {label}")
    model = GridPointInformerO3(
        seq_len=window,
        pred_len=horizon,
        lat_size=lat_size,
        lon_size=lon_size,
