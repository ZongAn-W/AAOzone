...
# 设置中文字体，确保散点图上的中文标签正常显示
plt.rcParams['font.sans-serif'] = ['SimHei', 'Arial Unicode MS'] # 兼容Windows和Mac
...
class PredRNNv2(nn.Module):
    def __init__(self, input_dim=10, hidden_dims=[64, 64, 64], height=H, width=W, horizon=horizon):
        super().__init__()

        # --- O3 参数 ---
        self.w1_o3 = nn.Parameter(torch.ones(1, 1, 1, height, width))
        self.w2_o3 = nn.Parameter(torch.ones(1, 1, 1, height, width))
        self.b1_o3 = nn.Parameter(torch.zeros(1, 1, 1, height, width))
        self.b2_o3 = nn.Parameter(torch.zeros(1, 1, 1, height, width))
        self.k1_o3 = nn.Parameter(torch.zeros(1, 1, 1, height, width)) # O3正弦扭曲参数
        self.k2_o3 = nn.Parameter(torch.zeros(1, 1, 1, height, width)) # O3余弦扭曲参数
    ...
    def forward(self, x, ls):
        ...
        k_safe_o1, k_safe_o2 = torch.tanh(self.k1_o3), torch.tanh(self.k2_o3)
        warp_o1 = k_safe_o1 * torch.cos(ls_rad)
        warp_o2 = k_safe_o2 * torch.sin(ls_rad)
        o3_sin = o3 * (self.w1_o3 * torch.sin(ls_rad + warp_o1 + self.b1_o3))
        o3_cos = o3 * (self.w2_o3 * torch.cos(ls_rad + warp_o2 + self.b2_o3))
        ...
        x_new = torch.cat([o3_sin, o3_cos, u_sin, u_cos, v_sin, v_cos, t_sin, t_cos, f_sin, f_cos], dim=2)
        ...
        for t in range(self.horizon):
            ...
            real_meteo = x_new[:, step_idx, 2:, :, :]  # 跳过 o3_sin/o3_cos，取 8 个气象通道
            ls_step = ls_rad[:, step_idx:step_idx+1, :, :, :]  
            
            warp_step_o1 = k_safe_o1 * torch.cos(ls_step)
            warp_step_o2 = k_safe_o2 * torch.sin(ls_step)
            current_o3_sin = current_o3 * (self.w1_o3 * torch.sin(ls_step + warp_step_o1 + self.b1_o3))
            current_o3_cos = current_o3 * (self.w2_o3 * torch.cos(ls_step + warp_step_o2 + self.b2_o3))
            
            inp = torch.cat([current_o3_sin.squeeze(1), current_o3_cos.squeeze(1), real_meteo], dim=1) 
            for i, cell in enumerate(self.layers):
                h[i], c[i], m = cell(inp, h[i], c[i], m)
                inp = h[i]
...
try:
    ...
    plt.title(f'真实值 VS 预测值 (带有相位扭曲 Phase Warping, Horizon={horizon})', fontsize=14, fontweight='bold')
    ...
    # 保存散点图
    scatter_save_path = os.path.join(base_dir, "models", "训练结果", "O3_Scatter_Plot_UVST.png")
    plt.savefig(scatter_save_path, dpi=300)
    print(f"✅ 散点图已保存至: {scatter_save_path}")
    plt.show()
