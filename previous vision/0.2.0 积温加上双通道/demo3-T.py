...
sys.stdout = Logger(os.path.join(base_dir, "models", "训练过程", "DualStream_ATTD.txt"))
...
dualstream_input_dim = 64
dualstream_num_layers = 3
dualstream_hidden_dims = [64, 64, 64]
...
def build_temperature_difference(temp_k):
    temp_k = np.asarray(temp_k, dtype=np.float32)
    temp_diff = np.zeros_like(temp_k, dtype=np.float32)
    # Use a backward difference so the fast branch stays causal at each step.
    temp_diff[1:] = temp_k[1:] - temp_k[:-1]
    return temp_diff
...
vars_dict['temp_diff'] = build_temperature_difference(vars_dict['temp'])
...
print(
    f"Temperature difference range: "
    f"{vars_dict['temp_diff'].min():.2f} ~ {vars_dict['temp_diff'].max():.2f}"
)

# ========================================
# 5. 构建 X, y 数据集
# ========================================
# 特征顺序: [O3_prev, accum_temp, temp, temp_diff]
X_raw = np.stack(
    [y_raw, vars_dict['accum_temp'], vars_dict['temp'], vars_dict['temp_diff']],
    axis=-1
)
...
class CausalLSTMCell(nn.Module):
    ...
    def forward(self, x, h, c, m):
        ...
        h_new = o_t * torch.tanh(self.conv_mem(merged_memory))
        return h_new, c_new, m_new
...
class PredRNN_DualStream(nn.Module):
    def __init__(
        ...
        # The 7x7 kernel keeps a wider receptive field so this branch learns
        # broad polar transport memory instead of reacting to local fast noise.
        ...
        # The 3x3 kernel keeps the fast branch focused on local gradients so
        # short-lived fluctuations do not directly pollute the slow memory path.
    ...
    def forward(self, x):
        B, T, C, H, W = x.shape
        if C != 4:
            raise ValueError("PredRNN_DualStream expects 4 channels ordered as [O3, AT, T, dT].")
        ...
        fused_feat = fused_feat.view(B, T, self.input_dim, H, W)

        h_states = [torch.zeros(B, d, H, W, device=x.device) for d in self.num_hidden]
        c_states = [torch.zeros_like(h_state) for h_state in h_states]
        memory = torch.zeros(B, self.num_hidden[-1], H, W, device=x.device)

        for t in range(T):
            current = fused_feat[:, t]
...
# 7. 训练循环 (GPU)
# ========================================
model = PredRNN_DualStream(
    input_dim=dualstream_input_dim,
    num_layers=dualstream_num_layers,
    num_hidden=dualstream_hidden_dims,
    horizon=horizon,
).to(device)
...
# 保存模型
torch.save(model.state_dict(), os.path.join(base_dir, "models", "训练结果", "predrnn_highlat_gpu_dualstream.pth"))
print("\n模型已保存: predrnn_highlat_gpu_dualstream.pth")

# ========================================
# 10. 变量–臭氧相关矩阵
# ========================================
print("\n[Analysis] Calculating Correlation Matrix...")
var_names = ['O3', 'AccumTemp', 'Temp', 'dT']
...
series = {
    ...
    'dT': vars_dict['temp_diff'].mean(axis=(1, 2)),
}
# 堆叠 (Time, N_vars)
...
try:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "models", "训练过程", "correlation_dualstream_attd.png"))
