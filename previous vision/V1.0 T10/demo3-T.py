...
o3_list = []
om_ls_list = []  # ★ 用于存储 OpenMars 的 Ls(太阳黄经)


# 修复: 使用自然排序(Natural Sort)
...
# ⚠️ 注意：如果你未来引入了太阳辐射通量，请将这部分归一化移到第 5 步（仅使用训练集的最大值），否则会造成轻微数据泄露。
...
# ----------------------------------------------------
# ★ 关键修复：严格遵守时间因果性，防止统计量泄露
# ----------------------------------------------------
# 1. 先定义时间截断点 (前 80% 的时间作为训练)
split_time_idx = int(0.8 * T)

# 2. 仅提取训练集数据用于计算统计量 (fit)
...
for i in range(T - window - horizon + 1):
    ...
    ls_seq.append(om_ls_continuous[i: i + window])  # ★ 截取对应的 Ls 序列

# 转为 Tensor
X_torch = torch.tensor(np.array(X_seq)).permute(0, 1, 4, 2, 3).float()
y_torch = torch.tensor(np.array(y_seq)).unsqueeze(2).float()
ls_torch = torch.tensor(np.array(ls_seq)).float()  # ★ 转换为 Tensor

# ----------------------------------------------------
# 5. 划分数据集
# ----------------------------------------------------
# 因为前面是取前 80% 时间作为训练，计算对应的样本索引点
split_sample_idx = split_time_idx - window - horizon + 1
split_sample_idx = max(0, min(split_sample_idx, len(X_torch)))  # 安全边界
...
class PredRNNv2(nn.Module):
    ...
    def forward(self, x, ls):
        ...
        # 将 Ls 角度转换为弧度，并扩展维度以支持广播机制 -> 形状变为 (B, T_dim, 1, 1, 1)
...
try:
    ...
    print("✅ 相关矩阵已保存为 correlation_UVPDST.png")
    plt.show()
