...
class PredRNNv2(nn.Module):
    # input_dim=9: O3_fused(1) + U_sin(1) + U_cos(1) + V_sin(1) + V_cos(1) + T_sin(1) + T_cos(1) + F_sin(1) + F_cos(1)
    def __init__(self, input_dim=9, hidden_dims=[64, 64, 64], height=H, width=W, horizon=horizon):
        super().__init__()

        # 将 w1_o3 初始化为 0，让加法调制开始时不干扰原始 O3，由网络慢慢学习调节幅度
    ...
    def forward(self, x, ls):
        ...
        for t in range(self.horizon):
            ...
            # 使用当前时刻的 ls 对 current_o3 进行加法调制
            ...
            inp = torch.cat([current_o3_fused.squeeze(1), real_meteo], dim=1)  # 重新拼接成 9 通道
            for i, cell in enumerate(self.layers):
                h[i], c[i], m = cell(inp, h[i], c[i], m)
                inp = h[i]
