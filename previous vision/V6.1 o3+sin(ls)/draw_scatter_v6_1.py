...
# 0. 基础路径与配置
# ========================================
# 当前脚本所在目录: d:\AAOzone\previous vision\V6.1 o3+sin(ls)
...
window, horizon = 3, 3 # V6.1 版本的 horizon 为 3
batch_size = 16
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

print(f"🚀 Using Device: {device} (Horizon={horizon})")
...
mcd_ls_list = []
target_files = [os.path.join(mcd_dir, f"MCD_MY{y}_Lat-90-90_real.nc") for y in [27, 28]]
...
class SeqDataset(torch.utils.data.Dataset):
    def __init__(self, X_scaled, y_scaled, ls_cont, start_idx, end_idx, window, horizon):
        self.X, self.y, self.ls = X_scaled, y_scaled, ls_cont
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(x_seq).permute(0, 3, 1, 2).float(), torch.from_numpy(ls_seq).float(), torch.from_numpy(y_seq).unsqueeze(1).float()

test_dataset = SeqDataset(X_scaled, y_scaled, om_ls_continuous, split_time_idx, T, window, horizon)
...
class SpatioTemporalLSTMCellv2(nn.Module):
    ...
    def forward(self, x, h, c, m):
        ...
        i_t, f_t, g_t = torch.sigmoid(i_x + i_h), torch.sigmoid(f_x + f_h + 1.0), torch.tanh(g_x + g_h)
        c_new = f_t * c + i_t * g_t
        i_tp, f_tp, g_tp = torch.sigmoid(i_xp + i_m), torch.sigmoid(f_xp + f_m + 1.0), torch.tanh(g_xp + g_m)
...
class PredRNNv2(nn.Module):
    def __init__(self, input_dim=9, hidden_dims=[64, 64, 64], height=H, width=W, horizon=horizon):
        super().__init__()
        for v in ['o3','u','v','t','f']:
            setattr(self, f'w1_{v}', nn.Parameter(torch.zeros(1,1,1,height,width) if v=='o3' else torch.ones(1,1,1,height,width)))
            setattr(self, f'b1_{v}', nn.Parameter(torch.zeros(1,1,1,height,width)))
            if v != 'o3':
                setattr(self, f'w2_{v}', nn.Parameter(torch.ones(1,1,1,height,width)))
                setattr(self, f'b2_{v}', nn.Parameter(torch.zeros(1,1,1,height,width)))
    ...
    def forward(self, x, ls):
        ...
        h = [torch.zeros(B, d, H_dim, W_dim, device=x.device) for d in self.hidden_dims]; c = [torch.zeros_like(hi) for hi in h]; m = torch.zeros_like(h[0])
        for t in range(T_encoder):
            inp = x_new[:, t]
            for i, cell in enumerate(self.layers): h[i], c[i], m = cell(inp, h[i], c[i], m); inp = h[i]
        preds = []; current_o3 = x[:, T_encoder-1 : T_encoder, 0:1]
...
with torch.no_grad():
    for xb, lsb, yb in test_loader:
        ...
        preds_all.append(model(xb, lsb).cpu().numpy())
...
y_true, y_pred = trues.flatten(), preds.flatten()

print("\n[Step 5] Plotting...")
plt.figure(figsize=(9, 8), facecolor='white')
hb = plt.hexbin(y_true, y_pred, gridsize=100, cmap='viridis', mincnt=1, bins='log')
max_val = max(np.max(y_true), np.max(y_pred))
min_val = min(np.min(y_true), np.min(y_pred))
plt.plot([min_val, max_val], [min_val, max_val], color='red', linestyle='--', lw=2, label='1:1 Line')
plt.xlabel('真实臭氧 (DU)', fontsize=12); plt.ylabel('预测臭氧 (DU)', fontsize=12)
plt.title('真实值 VS 预测值 (V6.1 horizon=3)', fontsize=14, fontweight='bold')
rmse = np.sqrt(np.mean((y_true - y_pred)**2))
r2 = 1 - (np.sum((y_true - y_pred)**2) / np.sum((y_true - np.mean(y_true))**2))
plt.text(0.05, 0.95, f'RMSE: {rmse:.4f}\n$R^2$: {r2:.4f}', transform=plt.gca().transAxes, verticalalignment='top', fontsize=12, bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
cb = plt.colorbar(hb); cb.set_label('样本密度 (log10)', fontsize=10)
plt.grid(True, linestyle=':', alpha=0.6); plt.legend(loc='lower right'); plt.tight_layout()
save_name = "O3_Scatter_Plot_V6.1.png"
