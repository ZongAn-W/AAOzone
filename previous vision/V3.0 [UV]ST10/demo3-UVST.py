...
class PredRNNv2(nn.Module):
    # input_dim 变为 7： O3(1) + UV_sin(1) + UV_cos(1) + T_sin(1) + T_cos(1) + Flux_sin(1) + Flux_cos(1)
    def __init__(self, input_dim=7, hidden_dims=[64, 64, 64], height=H, width=W, horizon=horizon):
        super().__init__()

        # ★ 新增：U和V通道的融合卷积层 (2个通道输入，1个通道输出)
        # 使用 1x1 卷积进行纯通道维度的特征混合
        self.uv_fusion = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=1)

        # 1. 融合后风场的调制参数
        self.w1_uv = nn.Parameter(torch.ones(1, 1, 1, height, width))
        self.w2_uv = nn.Parameter(torch.ones(1, 1, 1, height, width))
        self.b1_uv = nn.Parameter(torch.zeros(1, 1, 1, height, width))
        self.b2_uv = nn.Parameter(torch.zeros(1, 1, 1, height, width))
    ...
    def forward(self, x, ls):
        ...
        # ★ 核心修改：利用卷积层融合 U 和 V
        # 拼接 u 和 v，形状变为 (B, T_dim, 2, H, W)
        uv_cat = torch.cat([u, v], dim=2)
        # 折叠 B 和 T 维度送入 Conv2d：(B * T_dim, 2, H, W)
        uv_reshaped = uv_cat.view(B * T_dim, 2, H_dim, W_dim)
        # 卷积降维到 1 个通道，并恢复 B 和 T 维度：(B, T_dim, 1, H, W)
        uv_fused = self.uv_fusion(uv_reshaped).view(B, T_dim, 1, H_dim, W_dim)

        # 1. 调制 融合后的风场特征
        uv_sin = uv_fused * (self.w1_uv * torch.sin(ls_rad + self.b1_uv))
        uv_cos = uv_fused * (self.w2_uv * torch.cos(ls_rad + self.b2_uv))
        ...
        # 拼接全新的 7 通道输入张量
        x_new = torch.cat([o3, uv_sin, uv_cos, t_sin, t_cos, f_sin, f_cos], dim=2)
...
# ========================================
# 11. PFI (Permutation Feature Importance) 分析
# ========================================
...
for i, col_name in enumerate(feature_names):
    # 临时存储原始测试集数据以恢复
    # 注意：test_dataset 是 TensorDataset，包含 (X_torch, ls_torch, y_torch)
    # X_torch 形状: [Samples, Window, Channel, H, W]
    X_test_tensor = test_dataset.tensors[0].clone()
    ls_test_tensor = test_dataset.tensors[1]
    y_test_tensor = test_dataset.tensors[2]

    # 随机打乱当前特征的样本顺序 (保持空间结构，但打乱时间/样本对应关系)
    # 这样可以破坏该特征与目标值之间的物理联系
...
try:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "models", "训练结果", "PFI_Analysis.png"))
    print("✅ PFI 分析图表已保存。")
    plt.show()
except Exception as e:
    print(f"PFI 绘图失败: {e}")