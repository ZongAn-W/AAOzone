"""
这个脚本用于在火星臭氧预测任务上比较两组 TimeXer 风格实验：

1. TimeXer_Raw
   不使用相位扭曲模块，直接把历史原始变量送入 TimeXer 主干。
2. TimeXer_PhaseWarp
   先经过 PhaseWarpFrontEnd，再送入完全相同的 TimeXer 主干。

设计目标：
- 尽量保持与 dlinear_phasewarp_compare.py 相同的数据切分、标准化和评价口径；
- 采用“格点共享的点序列建模”方式，使 Transformer 类模型更符合其设计习惯；
- 保留 TimeXer 的核心思想：内生 patch 表示、外生变量 token、全局 token 桥接的 cross-attention；
...
class FlattenHead(nn.Module):
    """
    把每个变量对应的 patch 表示展平后，映射到未来 pred_len 步。

    输入：
    - x: [B_like, n_vars, d_model, patch_num_plus_one]
...
class EnEmbedding(nn.Module):
    ...
    def forward(self, x):
        # x: [B_like, n_vars, seq_len]
        batch_size, n_vars, seq_len = x.shape

        # 为了兼容任意 seq_len 和 patch_len，对末尾做最小复制填充。
        pad_len = (self.patch_len - seq_len % self.patch_len) % self.patch_len
        if pad_len > 0:
            x = torch.cat([x, x[:, :, -1:].repeat(1, 1, pad_len)], dim=-1)

        patch_num = x.shape[-1] // self.patch_len
        glb = self.glb_token[:, :n_vars].repeat(batch_size, 1, 1, 1)

        # [B_like, n_vars, seq_len] -> [B_like, n_vars, patch_num, patch_len]
        x = x.unfold(dimension=-1, size=self.patch_len, step=self.patch_len)
        x = x.reshape(batch_size * n_vars, patch_num, self.patch_len)

        x = self.value_embedding(x) + self.position_embedding(x)
        x = x.reshape(batch_size, n_vars, patch_num, self.d_model)
        x = torch.cat([x, glb], dim=2)

        # [B_like, n_vars, patch_num + 1, d_model] -> [B_like * n_vars, patch_num + 1, d_model]
        x = x.reshape(batch_size * n_vars, patch_num + 1, self.d_model)
        return self.dropout(x), n_vars, patch_num + 1
...
class ExEmbedding(nn.Module):
    ...
    这里采用变量级 token：
    - 每个变量的整个历史轨迹 [seq_len] 映射成一个 token
    - 后续由全局 token 对这些变量 token 做 cross-attention
    """
...
class TimeXerEncoderLayer(nn.Module):
    """
    TimeXer 风格编码层：
    - patch 序列内部做 self-attention
    - 最后一个全局 token 与变量 token 做 cross-attention
    - 再接前馈网络
    """
    ...
    def forward(self, x, cross):
        # x: [B_like * n_vars, patch_num_plus_one, d_model]
        # cross: [B_like * n_vars, n_vars, d_model]
        attn_out, _ = self.self_attention(x, x, x)
        x = self.norm1(x + attn_out)

        x_glb_ori = x[:, -1:, :]
        x_glb_attn, _ = self.cross_attention(x_glb_ori, cross, cross)
...
class TimeXerEncoder(nn.Module):
    ...
    def __init__(self, e_layers, d_model, n_heads, d_ff, dropout):
        super().__init__()
        self.layers = nn.ModuleList(
            [TimeXerEncoderLayer(d_model=d_model, n_heads=n_heads, d_ff=d_ff, dropout=dropout) for _ in range(e_layers)]
        )
        self.norm = nn.LayerNorm(d_model)
...
class TimeXerBackbone(nn.Module):
    ...
    - 内生变量走 patch 级表示
    - 外生变量走变量级 token 表示
    - 用全局 token 作为桥梁，让内生 patch 表示与外生变量 token 做交互
    """

    def __init__(self, seq_len, pred_len, n_vars, patch_len, d_model, n_heads, e_layers, d_ff, dropout, use_norm=True):
        super().__init__()
        ...
        self.patch_num_plus_one = (seq_len + pad_len) // patch_len + 1

        self.en_embedding = EnEmbedding(n_vars=n_vars, d_model=d_model, patch_len=patch_len, dropout=dropout)
        self.ex_embedding = ExEmbedding(seq_len=seq_len, d_model=d_model, dropout=dropout)
        self.encoder = TimeXerEncoder(e_layers=e_layers, d_model=d_model, n_heads=n_heads, d_ff=d_ff, dropout=dropout)
        self.head = FlattenHead(n_vars=n_vars, nf=d_model * self.patch_num_plus_one, pred_len=pred_len, dropout=dropout)
    ...
    def forward(self, x):
        ...
        en_embed, n_vars, _ = self.en_embedding(x.permute(0, 2, 1))
        ex_embed = self.ex_embedding(x)

        # 对于每个目标变量的 patch 序列，都复制一份同样的外生变量 token 集合作为 cross 输入。
        cross = ex_embed.repeat_interleave(n_vars, dim=0)
        enc_out = self.encoder(en_embed, cross)

        # [B_like * n_vars, patch_num_plus_one, d_model] -> [B_like, n_vars, d_model, patch_num_plus_one]
...
class GridPointTimeXerO3(nn.Module):
    """
    把每个经纬格点看成一条多变量时间序列，并共享同一个 TimeXer 模型。
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, patch_len, d_model, n_heads, e_layers, d_ff, dropout):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
...
def train_and_evaluate(
    ...
    """
    训练一组 TimeXer 实验，并返回评估指标。

    受控变量只有一个：是否启用 phase-warp 前端。
    """

    print(f"\n[Experiment] {label}")
    model = GridPointTimeXerO3(
        seq_len=window,
        pred_len=horizon,
        lat_size=lat_size,
        lon_size=lon_size,
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, "models", "训练过程", "TimeXer_PhaseWarp_Compare.txt"))
